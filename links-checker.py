...
def run(url, sitemap=False):
    ...
    print(f"Avi: {f'getting {rm_trailing_slash}...'.rjust(12)}")
    ...
    if req.status_code == 404:
        broken_link_msg = f"Avi: {'-'*10} {f'{rm_trailing_slash} returns {req.status_code}, Link is broken!'.rjust(12)}"
        print("*"*len(broken_link_msg))
        return print(broken_link_msg)
    if req.status_code >= 600:
        denied_link_msg = f"Avi: {'-'*10} {rm_trailing_slash} {f':::returns  {str(req.status_code)}'} {f' ::: link not permited for scanning by host ::: cant determine status'.rjust(15)}\n"
        print("*"*len(denied_link_msg))
        return print(denied_link_msg)
    print(f"Avi: {f'got {rm_trailing_slash}'.rjust(12)}")
    print(f"Avi: {f'parsing {rm_trailing_slash} content...'.rjust(12)}")
    soup = BeautifulSoup(req.content, 'html.parser')
    print(f"Avi: {f'{rm_trailing_slash} parsed successfully!'.rjust(12)}")
    links = soup.find_all('a')
    print(f"\nAvi:  {f'now searching for broken links in {rm_trailing_slash}'.rjust(12)}\n")
    broken_link = 0
    okay_links = 0
    denied_links = 0
    ...
    print(f"Avi: {f'analyzing {len(trimed_links)} links in {rm_trailing_slash}...'.rjust(12)}\n")
    try: 
        for link in trimed_links:
            ...
            try: 
                ...
                if get_link.raise_for_status() is not None:
                    print(f"Avi: {f'Connection timeout while trying to get {link}, status code {get_link.status_code}'.rjust(12)}")
                    continue
                ...
                if get_link.status_code >= 200 and get_link.status_code < 300:
                    print(f"Avi: link {url_to_check} {f':::returns  {str(get_link.status_code)}'.rjust(15)} {f' ::: link Okay'.rjust(15)}\n")
                    okay_links += 1
                    okay_links_arr.append(url_to_check)
            except Exception as e:
                if "Forbidden" in str(e):
                    exception_msg_text = f"Avi: {f'Error occured while trying to get {url_to_check}...::: Error :::...'.rjust(12)} {e}\n"
                    print(exception_msg_text)
                    denied_links += 1
                    denied_links_arr.append(url_to_check + " ::: reason :::}> " + str(e))
                elif "ConnectTimeoutError" in str(e):
                    exception_msg_text = f"Avi: {f'Error occured while trying to get {url_to_check}...::: Error :::...'.rjust(12)} Connection to {url_to_check} timed out. (connect timeout={25})\n"
                    print(exception_msg_text)
        ...
        print(f"Avi: {'Analysis'.rjust(12)}\n")
        analytics_log_file.write(f"Avi: {'Analysis'.rjust(12)}\n")
        if sitemap:
            ...
            general_file.write(f"Avi: {'Analysis'.rjust(12)}\n")
        ...
        print('Avi: Analysis interrupted!')
    if sitemap:
        general_file.close()
    ...
    try:
        if '--with-sitemap' in param:
            ...
            if link_to_xml_sitemap.startswith('http'):
                return print('Avi: Please provide a valid link to your xml sitemap!')
            print(f"Avi: {'Fetching ' + link_to_xml_sitemap + '...'.rjust(12)}")
            with open(link_to_xml_sitemap, 'r') as sitemap:
                ...
                print(f"Avi: {'Fetched ' + link_to_xml_sitemap + '!'.rjust(12)}\n")
                print(f"Avi: Total Links: {len(links)} {'Analyzing Sitemap '.rjust(12) + link_to_xml_sitemap + '...'}\n")
                try:
                    for link in links:
                        print(f"Avi: {'Analyzing URL '.rjust(12) + link.text + '...'}")
                        run(link.text, True)
                except KeyboardInterrupt as e:
                    print('Avi: Analysis interrupted!')
        else:
